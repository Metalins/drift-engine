...
async function loadBundle(agentId: string): Promise<AgentBundle> {
  const [agent, observables, probes, driftEvents] = await Promise.all([
    getAgent(agentId),
    getObservables(agentId, { limit: 50 }),
    getProbes(agentId, { status: "all", limit: 100 }),
    // #65 — drift-events timeline. Best-effort: a missing/erroring
    // endpoint (older server, transient failure) must not blank the
    // whole agent page, so it degrades to an empty timeline.
    getDriftEvents(agentId, { limit: 20 })
      .then((r) => r.items)
      .catch(() => [] as DriftEventRow[]),
  ]);
  return { agent, observables, probes, driftEvents };
}
...
export default async function AgentDetailPage({
  ...
  const { agent, observables, probes, driftEvents } = bundle;
  ...
  return (
    <main className="space-y-8">
      ...
      {/* Behavioral changes — #65 (κ-engine V2 drift timeline).
          The durable history of every DRIFT_DETECTED event the engine
          flagged for this agent. Distinct from the interactive
          DriftAlert above (which handles the CURRENT drift state): this
          is the read-only record. Collapsed by default; the summary
          line reflects whether behavior has been consistent. Hidden for
          empty agents — there's no traffic to have drifted yet. */}
      {state !== "empty" && (
        <CollapsedSection
          title="Behavioral changes"
          summary={
            driftEvents.length > 0
              ? `${driftEvents.length} change${
                  driftEvents.length === 1 ? "" : "s"
                } detected · most recent ${timeAgo(driftEvents[0].detected_at)}`
              : "Behavior consistent — no changes detected."
          }
          tone={driftEvents.length > 0 ? "info" : "ok"}
        >
          <BehavioralChanges events={driftEvents} />
        </CollapsedSection>
      )}
