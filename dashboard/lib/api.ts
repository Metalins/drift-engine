...
// --------------------------------------------------------------------------- //
// #64/#65 — behavioral drift events (κ-engine V2 alerts timeline)             //
// --------------------------------------------------------------------------- //
//
// Wraps GET /internal/v1/agents/{id}/drift-events. Each row is one
// DRIFT_DETECTED event the κ-engine V2 emitted when the agent's recent
// traffic drifted from its learned baseline. #65 renders these as the
// "Behavioral changes" timeline on the agent detail page.

export interface DriftEventRow {
  id: string;
  /** Raw engine feature name, e.g. "output_length_chars". Humanize for UI. */
...
export async function getDriftEvents(
  ...
  if (opts?.limit !== undefined) params.set("limit", String(opts.limit));
  const qs = params.toString() ? `?${params.toString()}` : "";
  return apiFetch<DriftEventsResponse>(
    `/v1/agents/${encodeURIComponent(agentId)}/drift-events${qs}`,
  );
}
